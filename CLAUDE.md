...
- Per-user state dir is `%LOCALAPPDATA%\dbxignore\` on Windows and `$XDG_STATE_HOME/dbxignore/` (fallback `~/.local/state/dbxignore/`) on Linux; the single source of truth is `state.user_state_dir()`, used by both `state.default_path()` and `daemon._log_dir()`. The v0.2-era Linux legacy-path fallback (`~/AppData/Local/dropboxignore/state.json`) was removed in v0.3 — clean-break upgrade path via `dropboxignore uninstall --purge` means no v0.2.x state survives, so the fallback had no remaining callers.
