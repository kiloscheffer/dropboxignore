...
- Daemon/sweep tests that trigger state writes: `monkeypatch.setattr(state, "default_path", lambda: tmp_path / "state.json")` redirects the persisted state off the real per-user state dir and keeps the test hermetic. Per-user state dir is `%LOCALAPPDATA%\dbxignore\` on Windows and `$XDG_STATE_HOME/dbxignore/` (fallback `~/.local/state/dbxignore/`) on Linux; the single source of truth is `state.user_state_dir()`, used by both `state.default_path()` and `daemon._log_dir()`. `state.read()` without an explicit path transparently falls back to the legacy Linux path (`~/AppData/Local/dbxignore/state.json`) for one release and logs WARNING; `state.read(path)` with an explicit path does not.
