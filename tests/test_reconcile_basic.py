...
def test_sets_ads_on_matching_directory(tmp_path, fake_ads, write_file):
...
def test_clears_ads_when_no_longer_matching(tmp_path, fake_ads, write_file):
    (tmp_path / "build").mkdir()
...
def test_no_ops_when_state_already_correct(tmp_path, fake_ads, write_file):
    write_file(tmp_path / ".dropboxignore", "build/\n")
    (tmp_path / "build").mkdir()
...
def test_matches_files_not_just_directories(tmp_path, fake_ads, write_file):
